#!/usr/bin/env python3
"""
Modulation classification with MoE — partial fine-tuning (PFT, no prompts).

This driver performs PFT by unfreezing the last two transformer layers of each
expert and training them jointly with router and classifier, with no prompt
tokens.
...
- MOD_MOE_TASK_EPOCHS:
    Task epochs (default "100").

- MOD_MOE_BATCH_SIZE:
    Batch size (default "32").

- MOD_MOE_MAX_SAMPLES_PER_CLASS:
    Max train samples per class; "0" means use all.

Outputs:
    REPO_ROOT / "outputs" / mod_class_moe_<dataset>_100ep_pft
unless overridden via MOD_MOE_RUN_NAME.
"""
...
DATASET_CONFIG: dict[str, tuple[str, str, str]] = {
    "ieee": ("datasets/spectrograms_ieee", "city_ieee_dataport", "mod_class_moe_ieee_100ep_pft"),
    "realworld_iq": ("datasets/spectrograms_realworld_iq", "city_realworld_iq", "mod_class_moe_realworld_iq_100ep_pft"),
    "radioml": ("datasets/spectrograms_radioml", "city_radioml", "mod_class_moe_radioml_100ep_pft"),
    "phoenix": ("datasets/spectrograms", "city_4_phoenix", "mod_class_moe_phoenix_100ep_pft"),
}
...
def _get_dataset() -> str:
    ...
    if env_ds in DATASET_CONFIG:
        return env_ds
    raise SystemExit(
        f"[mod_class_moe_pft] Please specify dataset via MOD_MOE_DATASET "
...
def _resolve_paths(dataset: str) -> tuple[Path, str, Path]:
    ...
    if not data_root.is_dir():
        raise FileNotFoundError(
            f"[mod_class_moe_pft] Spectrogram data not found: {data_root}. "
            f"For {dataset}, export to {rel_root}."
        )
    city_dir = data_root / city_name
    if not city_dir.is_dir():
        raise FileNotFoundError(f"[mod_class_moe_pft] City folder not found: {city_dir}.")

    comms_found = [c for c in ("LTE", "WiFi", "5G") if (city_dir / c).is_dir()]
    if not comms_found:
        raise FileNotFoundError(
            f"[mod_class_moe_pft] No comm folders (LTE, WiFi, 5G) under {city_dir}."
        )
    ...
    print(
        f"[mod_class_moe_pft] Dataset: {dataset} | data root: {data_root} | city: {city_name} | comms: {comms_found}"
    )
    print("[mod_class_moe_pft] Experts: partial fine-tuning (PFT, no prompts)")
...
def _check_experts() -> None:
    for name, p in [("LTE", LTE_CKPT), ("WiFi", WIFI_CKPT), ("5G", FIVE_G_CKPT)]:
        if not p.is_file():
            raise FileNotFoundError(f"[mod_class_moe_pft] Expert checkpoint not found for {name}: {p}")
...
def main() -> None:
    ...
    print(
        f"[mod_class_moe_pft] Config: epochs={task_epochs} patience={patience} "
    ...
    argv: list[str] = [
        ...
        str(patience),
        "--expert-lr",
        "5e-4",  # enables expert fine-tuning (last 2 transformer layers)
        "--expert",
